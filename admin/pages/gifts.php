        ...
        if(empty($points)) {
            ...
            switch($offer_type) {
                ...
                case 'addon':
                    $itemid1 = isset($_REQUEST['item_id']) ? $_REQUEST['item_id'] : null;
                    $count1 = isset($_REQUEST['item_count']) ? $_REQUEST['item_count'] : null;
                    $female_look_id = isset($_REQUEST['female_look_id']) ? $_REQUEST['female_look_id'] : null;
                    $male_look_id = isset($_REQUEST['male_look_id']) ? $_REQUEST['male_look_id'] : null;
                    $have_addon = isset($_REQUEST['have_addon']) ? $_REQUEST['have_addon'] : no;
                    if(!$itemid1 || !is_numeric($itemid1)) {
                        $errors[] = 'Please fill all fields. Item ID is empty or its not a number.';
                    }else if(!$count1 || !is_numeric($count1)) {
                        $errors[] = 'Please fill all fields. Item Count is empty or its not a number.';
                    }else if(!$female_look_id || !is_numeric($female_look_id)) {
                        $errors[] = 'Please fill all fields. Female Looktype ID is empty or its not a number.';
                    }else if(!$male_look_id || !is_numeric($male_look_id)) {
                        $errors[] = 'Please fill all fields. Male Looktype ID is empty or its not a number.';
                    }
                    break;
            ...
            if(empty($errors)) {
                $db->insert('z_shop_offer', array(
                    ...
                    'female_look_id' => $female_look_id,
                    'male_look_id' => $male_look_id,
                    'have_addon' => isset($have_addon) ? $have_addon : 'no'
                ));
