...
if($save)
{
	...
	if(config('account_create_character_create')) {
		...
		$ref_code = isset($_POST['code']) ? $_POST['code'] : null;
		$item_id_ref_code = null;

		if ($ref_code != null) {
			$valid = false;

			// percorre a lista de códigos configurados
			foreach ($config['codes'] as $c) {
				if ($c['code'] === $ref_code) {
					$valid = true;
					$item_id_ref_code = $c['itemid'];
					break;
				}
			}

			 if (!$valid) {
				$errors['code'] = "Ref Codigo invalido!";
			}
		}
	...
	if(empty($errors))
	{
		...
		else
		{
			if(config('account_create_character_create')) {
				// character creation
				$character_created = $createCharacter->doCreate($character_name, $character_sex, $character_vocation, $character_town, $new_account, $ref_code, $item_id_ref_code, $errors);
