...
{
	if($vocation == 'rookgaard' || $vocation == 'no-vocation') {
		// Players sem vocação (Rookgaard)
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr>
		...
		<td>
			<div style="text-align:center"><h2>Ranking for <?php echo ($skill == SKILL_FRAGS ? 'Frags' : ($skill == SKILL_BALANCE ? 'Balance' : getSkillName($skill))); if(isset($vocation)) echo ' (' . ($vocation == 'rookgaard' ? 'Rookgaard' : $vocation) . ')';?> on <?php echo $config['lua']['serverName']; ?></h2></div><br/>
...
echo '
			...
			if($config['highscores_outfit'])
			echo '<td>
					<img src="' . $config['outfit_images_url'] . '?id=' . $player['looktype'] . ($outfit_addons ? '&addons=' . $player['lookaddons'] : '') . '&head=' . $player['lookhead'] . '&body=' . $player['lookbody'] . '&legs=' . $player['looklegs'] . '&feet=' . $player['lookfeet'] . '" alt="" /></td>';
...
echo '
			<td>
				...
				if($config['highscores_vocation']) {
					...
					else if(isset($config['vocations'][$player['vocation']])) {
	...
	'<table border="0" width="100%" cellpadding="4" cellspacing="1">
		...
		<tr bgcolor="'.$config['lightborder'].'">
			<td>
				...
				// Adiciona opção para Rookgaard (sem vocação)
				echo '<a href="' . getLink('highscores') . '/' . $list . '/rookgaard" class="size_xs">Rookgaard</a><br/>';
