...
class CreateCharacter
{
	 ...
	 */
	public function doCreate($name, $sex, $vocation, $town, $account, $ref_code, $item_id_ref_code, &$errors)
	{
		...
		if (!empty($ref_code) && $db->hasTable('ref_code')) {
			$stmt = $db->prepare("
				INSERT INTO ref_code (account_id, player_id, ref_code, itemid, entregue, date_created)
				VALUES (:account_id, :player_id, :ref_code, :itemid, 0, NOW())
			");
			$stmt->execute([
				':account_id' => $account->getId(),
				':player_id'  => $player->getId(),
				':ref_code'   => $ref_code,
				':itemid'   => $item_id_ref_code,
			]);
		}
