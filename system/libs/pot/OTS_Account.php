 ...
 * @version 0.1.5
 */
    private $data = array('email' => '', 'rlname' => '','location' => '', 'country' => '','web_flags' => 0);
 ...
 */
	...
	// Buscar valores atuais do banco antes de salvar (para nao zerar premium)
	$currentData = $this->db->query('SELECT `lastday`, ' . 
		($this->db->hasColumn('accounts', 'premdays') ? '`premdays`, ' : '') . 
		($this->db->hasColumn('accounts', 'premend') ? '`premend`, ' : '') . 
		($this->db->hasColumn('accounts', 'premium_ends_at') ? '`premium_ends_at`, ' : '') . 
		'`created` FROM `accounts` WHERE `id` = ' . $this->data['id'])->fetch();
	
	// Preservar valores atuais se nao estiverem definidos
	if(!isset($this->data['lastday'])) {
		$this->data['lastday'] = isset($currentData['lastday']) ? $currentData['lastday'] : 0;
	}
	if(!isset($this->data['premdays']) && isset($currentData['premdays'])) {
		$this->data['premdays'] = $currentData['premdays'];
	}
	if(!isset($this->data['created'])) {
		$this->data['created'] = isset($currentData['created']) ? $currentData['created'] : 0;
	}
	...
	if($this->db->hasColumn('accounts', 'premend')) { // othire
		...
		if(!isset($this->data['premend'])) {
			$this->data['premend'] = isset($currentData['premend']) ? $currentData['premend'] : 0;
		}
	}
	else if($this->db->hasColumn('accounts', 'premium_ends_at')) {
		$field = 'premium_ends_at';
		if(!isset($this->data['premium_ends_at'])) {
			$this->data['premium_ends_at'] = isset($currentData['premium_ends_at']) ? $currentData['premium_ends_at'] : 0;
		}
	}
 ...
 */
	...
	{
		$account = $this->db->query('SELECT premium_ends_at FROM ' . $this->db->tableName('accounts') . ' WHERE id = '. $this->data['id'])->fetch();
        $premium_ends = $account['premium_ends_at'];
        if($premium_ends > time()){
            $date_now = $premium_ends - time();
            return floor($date_now / (24 * 60 * 60));
        }else{
            return 0;
        }
	}
